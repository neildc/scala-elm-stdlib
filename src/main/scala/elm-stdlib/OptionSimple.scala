...
object OptionSimple {
  ...
  def map3[A, B, C, Value](
    ...
    (optA, optB, optC) match {
      case (Some(a), Some(b), Some(c)) => Some(func(a, b, c))
      case _                           => None
    }
  ...
  def map4[A, B, C, D, Value](
    ...
    (optA, optB, optC, optD) match {
      case (Some(a), Some(b), Some(c), Some(d)) => Some(func(a, b, c, d))
      case _                                    => None
    }
  ...
  def map5[A, B, C, D, E, Value](
    ...
    (optA, optB, optC, optD, optE) match {
